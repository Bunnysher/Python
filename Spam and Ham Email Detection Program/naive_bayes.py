           ...
           # Calculates Term Frequency-Inverse Document Frequency based on user's input
            ...
            if IDF_option == 1:
                if all_wordfreq[words[j]] == 0:
                    ...
                    IDF = numpy.log10(len(train_files)
                            / all_wordfreq[words[j]])
            else:
                IDF = 1

           # Calculates the probability of being ham or spam, respectively

            word_ham.append(ham_wordfreq[words[j]] / float(len(ham))
                            * IDF + 1)
            word_spam.append(spam_wordfreq[words[j]] / float(len(spam))
                             * IDF + 1)
